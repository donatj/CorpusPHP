...
if( !$shutup ) :
	...
	$count    = (int)firstNotEmpty($this->data[1], $this->_config->TWEETCOUNT);
	...
	if( !$this->_cache->isExpired($cacheKey) ) {
		...
		if( $feed = file_get_contents($feedUrl) ) {
			$this->_cache->set($cacheKey, $feed, $this->_config->CACHEMINUTES, Cache::MINUTE, false);
		} else {
			$feed = $this->_cache->$cacheKey;
	...
	if( is_array($twitter) ) {
		...
		foreach( $twitter as $tweet ) {
			...
			if( $tweet['in_reply_to_status_id'] > 0 ) continue;

			$tweet['text'] = preg_replace('/([A-Za-z][A-Za-z0-9+.-]{1,120}:[A-Za-z0-9\/](([A-Za-z0-9$_.+!*,;\/?:@&~=-])|%[A-Fa-f0-9]{2}){1,333}(#([a-zA-Z0-9][a-zA-Z0-9$_.+!*,;\/?:@&~=%-]{0,1000}))?)/', '<a href="$1" target="_blank" class="' . $this->_config->LINKCLASS . '">$1</a>', $tweet['text']);
			$tweet['text'] = preg_replace('/@([A-Za-z0-9_]+)/', '<a href="http://twitter.com/$1" target="_blank" class="' . $this->_config->LINKCLASS . ' ' . $this->_config->USERCLASS . '">@$1</a>', $tweet['text']);
			echo '<p>';
			echo $tweet['text'];
			echo '<br /><small>' . co::module('blog/date', strtotime($tweet['created_at'])) .
