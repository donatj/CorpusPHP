 ...
 */
	function __initPluralRules() {
		...
		$coreIrregularPlural = array(
			...
			'turf' => 'turfs');
/*
		$pluralRules = array_unique(array_merge($this->__pluralRules, $corePluralRules));
		$uninflected = array_unique(array_merge( $this->__uninflectedPlural, $coreUninflectedPlural));
		$irregular = array_unique(array_merge($this->__irregularPlural, $coreIrregularPlural));
*/	
 ...
 */
	function pluralize($word) {
		...
		if (!isset($this->pluralRules) || empty($this->pluralRules)) {
			$this->__initPluralRules();
		}

		if (isset($this->pluralized[$word])) {
			return $this->pluralized[$word];
		}
		extract($this->pluralRules);

		if (!isset($regexUninflected) || !isset($regexIrregular)) {
			$regexUninflected = __enclose(join( '|', $uninflected));
			$regexIrregular = __enclose(join( '|', array_keys($irregular)));
			$this->pluralRules['regexUninflected'] = $regexUninflected;
			$this->pluralRules['regexIrregular'] = $regexIrregular;
		}

		if (preg_match('/(.*)\\b(' . $regexIrregular . ')$/i', $word, $regs)) {
			$this->pluralized[$word] = $regs[1] . substr($word, 0, 1) . substr($irregular[strtolower($regs[2])], 1);
			return $this->pluralized[$word];
		}

		if (preg_match('/^(' . $regexUninflected . ')$/i', $word, $regs)) {
			$this->pluralized[$word] = $word;
			return $word;
		}
